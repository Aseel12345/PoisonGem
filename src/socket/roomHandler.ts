import { Server, Socket } from 'socket.io';
import { RedisClientType } from 'redis';
import {
  Room, RoomPublic, Player,
  CreateRoomPayload, JoinRoomPayload, ReadyUpPayload,
  RematchVotePayload
} from '../types/game';
import { generateRoomCode, shuffleTurnOrder } from '../utils/crypto';

const ROOM_TTL_SECONDS = 60 * 60; // 1 hour — rooms expire from Redis after 1h of inactivity
const MIN_PLAYERS = 2;

// ─── Helpers ──────────────────────────────────────────────────────────────────

async function saveRoom(redis: RedisClientType, room: Room): Promise<void> {
  await redis.setEx(`room:${room.roomCode}`, ROOM_TTL_SECONDS, JSON.stringify(room));
}

async function getRoom(redis: RedisClientType, roomCode: string): Promise<Room | null> {
  const data = await redis.get(`room:${room}:${roomCode}`).catch(() => null);
  // fix: proper key
  const raw = await redis.get(`room:${roomCode}`);
  if (!raw) return null;
  return JSON.parse(raw) as Room;
}

async function deleteRoom(redis: RedisClientType, roomCode: string): Promise<void> {
  await redis.del(`room:${roomCode}`);
}

function toPublic(room: Room): RoomPublic {
  return {
    roomCode: room.roomCode,
    hostUid: room.hostUid,
    players: room.players.map(p => ({
      uid: p.uid,
      username: p.username,
      isAlive: p.isAlive,
      isReady: p.isReady,
      isConnected: p.isConnected,
    })),
    maxPlayers: room.maxPlayers,
    status: room.status,
    isPrivate: room.isPrivate,
    currentTurnUid: room.currentTurnUid,
    gems: room.gems,
    commitmentCount: room.poisonCommitments.length,
    winnerUid: room.winnerUid,
    roundNumber: room.roundNumber,
  };
}

// Build initial gem array — Phase 1 uses a flat list; patterns come in Phase 3
function buildGems(count: number) {
  return Array.from({ length: count }, (_, i) => ({
    index: i,
    isEliminated: false,
  }));
}

// Gem count = players * 2 (gives meaningful choices without dragging too long)
function gemCountForPlayers(playerCount: number): number {
  return Math.max(playerCount * 2, 6);
}

// ─── Room Handlers ────────────────────────────────────────────────────────────

export function registerRoomHandlers(
  io: Server,
  socket: Socket,
  redis: RedisClientType
) {
  // ── Create Room ──────────────────────────────────────────────────────────────
  socket.on('room:create', async (payload: CreateRoomPayload) => {
    try {
      const { uid, username, isPrivate, maxPlayers } = payload;

      if (maxPlayers < MIN_PLAYERS || maxPlayers > 10) {
        socket.emit('error', { code: 'INVALID_MAX_PLAYERS', message: 'Max players must be 2–10' });
        return;
      }

      const roomCode = generateRoomCode();

      const host: Player = {
        uid, username, socketId: socket.id,
        isAlive: true, isReady: false, isConnected: true,
      };

      const room: Room = {
        roomCode,
        hostUid: uid,
        players: [host],
        maxPlayers,
        status: 'waiting',
        isPrivate,
        currentTurnUid: null,
        turnOrder: [],
        gems: [],
        poisonCommitments: [],
        afkTimerStart: null,
        winnerUid: null,
        roundNumber: 0,
        createdAt: Date.now(),
      };

      await saveRoom(redis, room);
      await socket.join(roomCode);

      socket.emit('room:created', { room: toPublic(room) });
      console.log(`[Room] ${username} created room ${roomCode}`);
    } catch (err) {
      console.error('[room:create]', err);
      socket.emit('error', { code: 'SERVER_ERROR', message: 'Failed to create room' });
    }
  });

  // ── Join Room ────────────────────────────────────────────────────────────────
  socket.on('room:join', async (payload: JoinRoomPayload) => {
    try {
      const { uid, username, roomCode } = payload;
      const room = await getRoom(redis, roomCode);

      if (!room) {
        socket.emit('error', { code: 'ROOM_NOT_FOUND', message: 'Room not found' });
        return;
      }
      if (room.status !== 'waiting') {
        socket.emit('error', { code: 'GAME_IN_PROGRESS', message: 'Game already started' });
        return;
      }
      if (room.players.length >= room.maxPlayers) {
        socket.emit('error', { code: 'ROOM_FULL', message: 'Room is full' });
        return;
      }

      // Handle rejoin (e.g. after disconnect)
      const existingIdx = room.players.findIndex(p => p.uid === uid);
      if (existingIdx !== -1) {
        room.players[existingIdx].socketId = socket.id;
        room.players[existingIdx].isConnected = true;
      } else {
        const player: Player = {
          uid, username, socketId: socket.id,
          isAlive: true, isReady: false, isConnected: true,
        };
        room.players.push(player);
      }

      await saveRoom(redis, room);
      await socket.join(roomCode);

      // Tell the joiner their room state
      socket.emit('room:joined', { room: toPublic(room) });
      // Tell everyone else
      socket.to(roomCode).emit('room:updated', { room: toPublic(room) });

      console.log(`[Room] ${username} joined room ${roomCode}`);
    } catch (err) {
      console.error('[room:join]', err);
      socket.emit('error', { code: 'SERVER_ERROR', message: 'Failed to join room' });
    }
  });

  // ── Ready Up ─────────────────────────────────────────────────────────────────
  socket.on('room:ready', async (payload: ReadyUpPayload) => {
    try {
      const { uid, roomCode } = payload;
      const room = await getRoom(redis, roomCode);

      if (!room || room.status !== 'waiting') return;

      const player = room.players.find(p => p.uid === uid);
      if (!player) return;

      player.isReady = true;
      await saveRoom(redis, room);

      io.to(roomCode).emit('room:updated', { room: toPublic(room) });

      // Auto-start if all players (min 2) are ready
      const allReady = room.players.length >= MIN_PLAYERS &&
        room.players.every(p => p.isReady);

      if (allReady) {
        // Transition to poisoning phase — game handler takes over
        room.status = 'poisoning';
        room.roundNumber += 1;
        room.gems = buildGems(gemCountForPlayers(room.players.length));
        room.turnOrder = shuffleTurnOrder(room.players.map(p => p.uid));
        room.poisonCommitments = [];
        await saveRoom(redis, room);

        io.to(roomCode).emit('game:poisoning_phase', { room: toPublic(room) });
        console.log(`[Room] ${roomCode} — all ready, entering poisoning phase`);
      }
    } catch (err) {
      console.error('[room:ready]', err);
    }
  });

  // ── Leave Room ───────────────────────────────────────────────────────────────
  socket.on('room:leave', async ({ uid, roomCode }: { uid: string; roomCode: string }) => {
    await handlePlayerLeave(io, socket, redis, uid, roomCode, false);
  });

  // ── Disconnect ───────────────────────────────────────────────────────────────
  socket.on('disconnect', async () => {
    // Find rooms this socket is in and mark player disconnected/eliminated
    // We store socket→room mapping in Redis for fast lookup
    const mapping = await redis.get(`socket:${socket.id}`);
    if (!mapping) return;

    const { uid, roomCode } = JSON.parse(mapping) as { uid: string; roomCode: string };
    await handlePlayerLeave(io, socket, redis, uid, roomCode, true);
    await redis.del(`socket:${socket.id}`);
  });

  // ── Track socket→room mapping when joining ───────────────────────────────────
  socket.on('room:register_socket', async ({ uid, roomCode }: { uid: string; roomCode: string }) => {
    await redis.setEx(`socket:${socket.id}`, ROOM_TTL_SECONDS, JSON.stringify({ uid, roomCode }));
  });

  // ── Rematch Vote ─────────────────────────────────────────────────────────────
  socket.on('room:rematch_vote', async (payload: RematchVotePayload) => {
    try {
      const { uid, roomCode } = payload;
      const room = await getRoom(redis, roomCode);
      if (!room || room.status !== 'finished') return;

      // Store votes in Redis separately
      await redis.sAdd(`rematch:${roomCode}`, uid);
      const votes = await redis.sCard(`rematch:${roomCode}`);
      const totalAlive = room.players.filter(p => p.isConnected).length;

      io.to(roomCode).emit('room:rematch_votes', { votes, total: totalAlive });

      // Majority vote triggers rematch
      if (votes >= Math.ceil(totalAlive / 2)) {
        await redis.del(`rematch:${roomCode}`);
        // Reset room
        room.status = 'waiting';
        room.players = room.players.map(p => ({ ...p, isAlive: true, isReady: false }));
        room.gems = [];
        room.poisonCommitments = [];
        room.currentTurnUid = null;
        room.turnOrder = [];
        room.winnerUid = null;
        room.afkTimerStart = null;
        await saveRoom(redis, room);
        io.to(roomCode).emit('room:rematch_start', { room: toPublic(room) });
      }
    } catch (err) {
      console.error('[room:rematch_vote]', err);
    }
  });
}

// ─── Shared Leave Logic ───────────────────────────────────────────────────────

async function handlePlayerLeave(
  io: Server,
  socket: Socket,
  redis: RedisClientType,
  uid: string,
  roomCode: string,
  isDisconnect: boolean
) {
  const room = await getRoom(redis, roomCode);
  if (!room) return;

  const playerIdx = room.players.findIndex(p => p.uid === uid);
  if (playerIdx === -1) return;

  const player = room.players[playerIdx];

  if (room.status === 'waiting') {
    // Just remove from room
    room.players.splice(playerIdx, 1);
    if (room.players.length === 0) {
      await deleteRoom(redis, roomCode);
      return;
    }
    // Transfer host if needed
    if (room.hostUid === uid && room.players.length > 0) {
      room.hostUid = room.players[0].uid;
    }
    await saveRoom(redis, room);
    io.to(roomCode).emit('room:updated', { room: toPublic(room) });
  } else if (room.status === 'playing' || room.status === 'poisoning') {
    // Mid-game: eliminate them immediately
    player.isAlive = false;
    player.isConnected = false;

    io.to(roomCode).emit('game:player_disconnected', {
      uid,
      username: player.username,
    });

    // Check win condition
    const alivePlayers = room.players.filter(p => p.isAlive);
    if (alivePlayers.length === 1) {
      room.status = 'finished';
      room.winnerUid = alivePlayers[0].uid;
      await saveRoom(redis, room);
      io.to(roomCode).emit('game:over', {
        winnerUid: room.winnerUid,
        winnerUsername: alivePlayers[0].username,
        poisonReveals: room.poisonCommitments.map(c => ({
          uid: c.uid,
          gemIndex: c.gemIndex,
          salt: c.salt,
          commitHash: c.commitHash,
          username: room.players.find(p => p.uid === c.uid)?.username ?? '',
        })),
      });
    } else if (alivePlayers.length === 0) {
      room.status = 'finished';
      room.winnerUid = null;
      await saveRoom(redis, room);
      io.to(roomCode).emit('game:over', { winnerUid: null, winnerUsername: null, poisonReveals: [] });
    } else {
      // If it was this player's turn, advance turn
      if (room.currentTurnUid === uid) {
        const aliveUids = alivePlayers.map(p => p.uid);
        const nextUid = getNextTurn(room.turnOrder, uid, aliveUids);
        room.currentTurnUid = nextUid;
        room.afkTimerStart = Date.now();
      }
      await saveRoom(redis, room);
      io.to(roomCode).emit('room:updated', { room: toPublic(room) });
    }
  }

  socket.leave(roomCode);
}

function getNextTurn(turnOrder: string[], currentUid: string, aliveUids: string[]): string {
  const idx = turnOrder.indexOf(currentUid);
  for (let i = 1; i <= turnOrder.length; i++) {
    const next = turnOrder[(idx + i) % turnOrder.length];
    if (aliveUids.includes(next)) return next;
  }
  return aliveUids[0];
}

export { getRoom, saveRoom, toPublic, getNextTurn };
